...
(* type transformation = { deltaX : float; deltaY : float; (*deltaZ: float; *) (* translation *) *)

type rotation =	{ theta : int; phi : int }                   (* rotation    *)
(*	zoom : float }                                      (* scaling     *) *)


(* calcule nouveaux x, y et z *)
(* NB si phi = 0, il n'y aura pas de ligne, donc il faut avoir phi = 90 par defaut *)
let deplace point dist rotation scale_factor =
  let scaled_dist = (float_of_int dist) /. scale_factor in
  let new_x = point.x +.(scaled_dist *. (cos (rad_of_deg rotation.theta)) *. (sin (rad_of_deg rotation.phi)))
  and new_y = point.y +.(scaled_dist *. (sin (rad_of_deg rotation.theta)) *. (sin (rad_of_deg rotation.phi)))
  and new_z = point.z +.(scaled_dist *. (cos (rad_of_deg rotation.phi)))
  in { x = new_x; y = new_y; z = new_z };;



(* retourne les noueaux x et y plus un maxmin altere si neccessaire *)
let deplace_compare point dist rotation scale_factor maxmin_xy =
  let new_point = deplace point dist rotation scale_factor
...
let rec execute point rotation minmax_xy scale_factor instructions check =
  match instructions with
  | [] -> (point,rotation,minmax_xy)
  | instruction::instructions' ->
    match instruction with
    | Line(dist) ->
      begin
        let new_point = deplace point dist rotation scale_factor in
        ...
        execute new_point rotation new_minmax_xy scale_factor instructions' check
      end
    | Move(dist) ->
      begin
        let new_point = deplace point dist rotation scale_factor in
    ...
    | Turn(deg) ->
      begin
        let newrotation = { theta = (rotation.theta + deg) mod 360; phi = rotation.phi } in
        execute point newrotation minmax_xy scale_factor instructions' check
      end
    | TurnPhi (deg) ->
      begin
	let newrotation = { theta = rotation.theta; phi = 180 - abs (((phi + deg) mod 360) - 180)} in
    ...
    | Rectangle(largeur, hauteur) ->
      begin
        ...
        let new_point = deplace point largeur rotation scale_factor in
        moveto (int_of_float new_point.x) (int_of_float new_point.y);
        let new_minmax_xy = if check then calc_new_minmax new_point minmax_xy else minmax_xy in
        execute new_point rotation new_minmax_xy scale_factor instructions' check
      end
    | MoveNoScale(dist) ->
      begin
        let new_point = deplace point dist rotation 1.0 in
        moveto (int_of_float new_point.x) (int_of_float new_point.y);
        let new_minmax_xy = if check then calc_new_minmax new_point minmax_xy else minmax_xy in
        execute new_point rotation new_minmax_xy scale_factor instructions' check
      end
